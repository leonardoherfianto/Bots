...
const TARGET_SHEET = '02 Analysis Youtube'; 
...
// Helper: Fetch API Statistik Video
...
// Membaca target sheet untuk mengetahui baris dan waktu update terakhir
async function getExistingDataMap() {
    ...
    try {
        ...
        console.log("Catatan: Sheet Target kosong / belum bisa dibaca.");
        return new Map();
    }
}
...
async function runYouTubeBot() {
    try {
        console.log("📥 [YouTube] Mengambil daftar target dari Spreadsheet '01 Listing'...");
        ...
        // Urutkan dari yang paling lama tidak diupdate (angka terkecil) ke yang terbaru
        dataset.sort((a, b) => a.lastUpdateMs - b.lastUpdateMs);

        console.log("🌐 Menyiapkan environment YouTube...");
        ...
        for (let item of dataset) {
            ...
            try {
                ...
                if (!targetUrl.endsWith('/videos')) targetUrl = targetUrl.replace(/\/$/, '') + '/videos';
                ...
                const scrapeResult = await page.evaluate(() => {
                    try {
                        ...
                        const videoTab = tabs?.find(t => t.tabRenderer?.title === 'Videos' || t.tabRenderer?.title === 'Video');
                        
                        if (videoTab) {
                            const contents = videoTab.tabRenderer.content?.richGridRenderer?.contents;
                            if (contents) {
                                contents.forEach(c => {
                                    const vRender = c.richItemRenderer?.content?.videoRenderer;
                                    if (vRender && vRender.videoId) videoIds.push(vRender.videoId);
                                });
                            }
                        }
                ...
                if (!scrapeResult || !scrapeResult.channelId) {
                    console.log("   ❌ Gagal scraping ID dari halaman.");
                    continue;
                }
                ...
                // Simpan ke spreadsheet (Update jika sudah ada, Append jika baru)
            ...
            await sleep(3000 + Math.random() * 2000); // Delay anti rate-limit
        }

        console.log("\n🏁 Semua target YouTube Video selesai diproses.");
        browser.disconnect();
